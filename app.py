...
app.config["SESSION_TYPE"] = "filesystem"  # or use "redis" if Redis is available
Session(app)
...
def index():
    if request.method == "POST":
        ...
        session["beta1"] = beta1
        session["S"] = S
        print("Session data after generation:", dict(session))
...
def hypothesis_test():
    ...
    plt.axvline(hypothesized_value, color="blue", linestyle="-", label=f"Hypothesized {parameter.capitalize()} (H₀): {hypothesized_value:.4f}")
...
def confidence_interval():
    ...
    ci_bounds = stats.t.interval(confidence_level, len(estimates) - 1, loc=mean_estimate, scale=std_estimate / np.sqrt(len(estimates)))
    ...
    plt.hlines(0, ci_lower, ci_upper, color="blue", linewidth=3, label=f"{confidence_level*100:.1f}% Confidence Interval")
